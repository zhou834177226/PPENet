...
def cal_accuracy(model, dataloadr, att, test_id, device, bias=None):
    ...
    for iteration, (img, label) in enumerate(dataloadr):
        ...
        # feature3, feature4, score = model(img, seen_att=att)
        # out['f3'].append(feature3)
        # out['f4'].append(feature4)
        for i in range(0, 11):
            scores[i].append(score[i])
        labels.append(label)

    # out['f3'] = torch.cat(out['f3'], dim=0)
    # out['f4'] = torch.cat(out['f4'], dim=0)
    # out['labels'] = labels
    # torch.save(out, 'E:/PROJECT/Gem/config/GEMZSL/features.pth')
